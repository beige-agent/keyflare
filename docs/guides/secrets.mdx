...
# As YAML
kfl download --project my-api --config production --format yaml
...
```bash
kfl secrets delete OLD_KEY --project my-api --config production
```
...
How it works:
1. Fetches all secrets for the specified project/config
