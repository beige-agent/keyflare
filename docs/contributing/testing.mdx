...
| `test/api.test.ts` | Full API integration tests: keys CRUD, projects, configs, secrets, system-key scoping |
...
it('system key respects scopes', async () => {
  ...
  const allowedResponse = await worker.fetch(
    new Request('http://localhost/projects/my-api/configs/production/secrets', {
  ...
  const deniedResponse = await worker.fetch(
    new Request('http://localhost/projects/other/configs/dev/secrets', {
      headers: { Authorization: `Bearer ${sysKey}` }
    }),
    env,
    ctx
